...
public class HrDocumentRequestsFacade extends AbstractFacade<HrDocumentRequests> {
    ...
    public List<HrDocumentRequests> findByRequesterId(HrEmployees Requester) {
        Query query = em.createNamedQuery("HrDocumentRequests.findByRequesterId", HrDocumentRequests.class);
        query.setParameter("reqId", Requester.getId());
        List<HrDocumentRequests> qualList = new ArrayList(query.getResultList());
        return qualList;
    }
    ...
    public List<HrDocumentRequests> findAllRequests(int preparestatus) {
        Query query = em.createNamedQuery("HrDocumentRequests.findByStatus");
        query.setParameter("status", preparestatus);
        try {
            return (List<HrDocumentRequests>) query.getResultList();
        } catch (Exception ex) {
            return null;
