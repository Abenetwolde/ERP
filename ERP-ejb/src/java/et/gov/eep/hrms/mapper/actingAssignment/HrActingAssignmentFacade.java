...
//        query.setParameter("requestDate", hrActingAssignments.getRequestDate() + '%');
    ...
    public boolean isRequstExist(HrActingAssignments actingAssignment) {
        ...
        query.setParameter("empId", actingAssignment.getAssignedBy().getId());
        try {
    ...
    public List<HrActingAssignments> loadEmpActingAssignmentHistoty(HrEmployees hrEmployees) {
        Query query = em.createNamedQuery("HrActingAssignments.LoadEmpActingHistory");
        System.out.println("hrEmployees.getId()==="+hrEmployees.getId());
