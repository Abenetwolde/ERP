...
public class HrSmCompetencyTypesFacade extends AbstractFacade<HrSmCompetencyTypes> {
    ...
    public List<HrSmCompetencyTypes> findBySkill(String skill) {
        Query query = em.createNamedQuery("HrSmCompetencyTypes.findBySkill", HrSmCompetencyTypes.class);
        query.setParameter("competencyId", skill);
        try {

            List<HrSmCompetencyTypes> skillCompList = new ArrayList(query.getResultList());
            return skillCompList;
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
