...
public class MmsFixedassetRegstrationFacade extends AbstractFacade<MmsFixedassetRegstration> {
    ...
    public HrEmployees findById(HrEmployees employeeFirst) {
       em.getEntityManagerFactory().getCache().evictAll();
        Query query = em.createNamedQuery("HrEmployee.findByEmpId");
        query.setParameter("empId",  employeeFirst.getEmpId());
        System.out.println("====empId==="+employeeFirst.getEmpId());
        try {
//            if(query.getResultList().size() > 0){
//          HrEmployee empl = (HrEmployee) query.getResultList().get(0);
//            }
            HrEmployees emplId = (HrEmployees) query.getResultList().get(0);
            return emplId;
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public ArrayList<MmsFixedassetRegstDetail> regstrationsucs(String empId) {
        Query q = em.createNamedQuery("HrEmployee.findByFirstNameLikes", MmsFixedassetRegstration.class);
        q.setParameter("empId", empId + '%');
        try {
            ArrayList<MmsFixedassetRegstDetail> employeeInformations = new ArrayList(q.getResultList());
     ...
     public ArrayList<MmsFixedassetRegstration> regstrationsuc(String regstrationuc) {
        Query q = em.createNamedQuery("HrEmployee.findByFirstNameLikes", MmsFixedassetRegstration.class);
        q.setParameter("empId", regstrationuc + '%');
        try {
            ArrayList<MmsFixedassetRegstration> employeeInformations = new ArrayList(q.getResultList());
            return employeeInformations;
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
