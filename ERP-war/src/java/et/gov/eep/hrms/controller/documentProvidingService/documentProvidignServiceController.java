...
//</editor-fold>
    ...
    private boolean chooseMainPage1 = true;
    ...
    private boolean DisabledEmpId = false;
    ...
    private int preparestatus = Constants.PREPARE_VALUE;
    private int checkerApprovestatsu = Constants.CHECK_APPROVE_VALUE;
    private int checkerRejectstatus = Constants.CHECK_REJECT_VALUE;
    private int approveStatus = Constants.APPROVE_VALUE;
    private int approverRejectstatus = Constants.APPROVE_REJECT_VALUE;
...
//</editor-fold>
    ...
    List<HrDocumentRequests> requesstHistory = new ArrayList<>();
...
//</editor-fold>
    ...
    public void init() {
        ...
        if (workFlow.isRequestStatus()) {
            System.out.println("requester");
            DisabledEmpId = true;
            Requester.setId((sessionBeanLocal.getEmpId()));
            Requester = hrEmployeeBean.searchById(Requester);
            hrDocumentRequests.setRequesterId(Requester);
            hrDepartments = Requester.getDeptId();
            hrJobTypes = Requester.getJobId();
            exprienceCalc(Requester.getHireDate());
            requesstHistory = documentProvidingServiceBeanLocal.findByRequesterId(Requester);
            chooseMainPage = false;
            chooseMainPage1 = false;
            chooseCreatePage = true;
            btnNewRender = false;
            headertext = "Document Request";

        } else {
            Requester = new HrEmployees();
            chooseMainPage = true;
            chooseCreatePage = false;
            chooseMainPage1 = true;
            btnNewRender = false;
        }
    }
//</editor-fold>
    ...
    public List<HrDocumentRequests> getRequesstHistory() {
        return requesstHistory;
    }

    public void setRequesstHistory(List<HrDocumentRequests> requesstHistory) {
        this.requesstHistory = requesstHistory;
    }
    ...
    public boolean isChooseMainPage1() {
        return chooseMainPage1;
    }

    public void setChooseMainPage1(boolean chooseMainPage1) {
        this.chooseMainPage1 = chooseMainPage1;
    }
    ...
    public int getCheckerApprovestatsu() {
        return checkerApprovestatsu;
    }

    public int getCheckerRejectstatus() {
        return checkerRejectstatus;
    }
    ...
    public int getApproverRejectstatus() {
        return approverRejectstatus;
    }
    ...
    public boolean isDisabledEmpId() {
        return DisabledEmpId;
    }

    public void setDisabledEmpId(boolean DisabledEmpId) {
        this.DisabledEmpId = DisabledEmpId;
    }
...
//    }
    public List<SelectItem> getFilterByStatus() {
        ...
        selectItems.add(new SelectItem(String.valueOf(Constants.PREPARE_VALUE), "Load Request List"));
        selectItems.add(new SelectItem(String.valueOf(Constants.APPROVE_VALUE), "Load Approved List"));
        selectItems.add(new SelectItem(String.valueOf(Constants.APPROVE_REJECT_VALUE), "Load Rejected List"));
    ...
    public int getRequestNotificationCounter() {
        requesstlist = documentProvidingServiceBeanLocal.findAllrequests(Constants.PREPARE_VALUE);
...
//</editor-fold>
    ...
    private void populateTable() {
        try {
            ...
            List<HrDocumentRequests> readFilteredRetirement = documentProvidingServiceBeanLocal.loadDocumentRequests(status);
    ...
    public String exprienceCalc(String hiredate) {
        String Shday[] = Requester.getHireDate().split("/");
        int Ihday = Integer.parseInt(Shday[0]);
        String Shmonth[] = Requester.getHireDate().split("/");
        int Ihmonth = Integer.parseInt(Shmonth[1]);
        ...
        if (Requester.getId() == null) {
            ...
            try {
                ...
                if (security.checkAccess(sessionBeanLocal.getUserName(), "saveDocumentApprove", dataset)) {
                    if (selectedValue.equalsIgnoreCase(String.valueOf(approveStatus))) {
                        hrDocumentRequests.setStatus(approveStatus);
                        documentProvidingServiceBeanLocal.update(hrDocumentRequests);
                        System.out.println("entered here 1");
                        wfSave();
                        JsfUtil.addSuccessMessage("Request Successfully Approved");
                        selectedValue = null;
                        ClearDocumentRequests();
                        chooseCreatePage = false;
                        chooseMainPage = true;
                    } else if (selectedValue.equalsIgnoreCase(String.valueOf(approverRejectstatus))) {
                        hrDocumentRequests.setStatus(approverRejectstatus);
                        documentProvidingServiceBeanLocal.update(hrDocumentRequests);
                        System.out.println("entered here 2");
                        wfSave();
                        JsfUtil.addSuccessMessage("Request Successfully rejected ");
                        selectedValue = null;
                        ClearDocumentRequests();
                        chooseCreatePage = false;
                        chooseMainPage = true;
...
//..... add more information by calling fields defined in the object 
    ...
    public void docTypelistner(ValueChangeEvent event) {
        if (event.getNewValue() != null) {
            hrDocumentTypes.setId(Integer.valueOf(event.getNewValue().toString()));
            System.out.println("hrDocumentTypes===" + hrDocumentTypes);
            hrDocumentTypes = documentTypeBeanLocal.getById(hrDocumentTypes);
            hrDocumentRequests.setDocumentType(hrDocumentTypes);
            if (hrDocumentTypes.getId() == 3) {
                guranteepnl = true;
            } else {
                guranteepnl = false;
            }
