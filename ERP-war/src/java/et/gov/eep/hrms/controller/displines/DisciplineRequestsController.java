...
// </editor-fold> 
    ...
    public void init() {
        ...
        if (workFlow.isRequestStatus()) {
            System.out.println("I am requester");
            disabledEmpId = true;
            hrEmployeesRequester.setId((sessionBeanLocal.getEmpId()));
            hrEmployeesRequester = hrEmployeeBeanLocal.searchById(hrEmployeesRequester);
...
//            }
    ...
    public void populate(SelectEvent events) {
        ...
        localBeanObj.setSaveOrUpadetStatus(1);
        update = 1;
        renderPnlManPage = false;
        renderPnlCreateAdditional = true;
        btnNewRender = true;
...
//</editor-fold>
    ...
    public ArrayList<HrEmployees> searchEmployeeByIdForRequester(String empId) {
        ArrayList<HrEmployees> employee = null;
        hrEmployeesRequester.setEmpId(empId);
        employee = hrEmployeeBean.SearchByEmpId(hrEmployeesRequester);
        return employee;
    }

    public void getByIdForRequester(SelectEvent event) {
        try {
            hrEmployeesRequester.setEmpId(event.getObject().toString());
            hrEmployeesRequester = hrEmployeeBean.getByEmpId(hrEmployeesRequester);
            departmentsOfAccused = hrEmployeesRequester.getDeptId();
            hrJobTypesOfAccused = hrEmployeesRequester.getJobId();
            disciplineRequests.setRequesterId(hrEmployeesRequester);
        } catch (Exception ex) {
